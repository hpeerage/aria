...
export default function WishlistPanel() {
  ...
  const handleExportPDF = async () => {
    if (!ticketRef.current || wishlist.length === 0) return;
    
    setIsExporting(true);
    
    try {
      // 1. Wait a bit for images to load if needed (though browser usually has them)
      await new Promise(resolve => setTimeout(resolve, 500));
      
      // 2. Capture the ticket component
      ...
      pdf.save("aria_wellness_journey.pdf");
              ...
              {wishlist.length > 0 && (
                <div className="p-6 border-t border-forest/5 dark:border-white/5 bg-forest/[0.02] dark:bg-white/[0.02] space-y-3 print:hidden">
                   <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 py-4 bg-forest dark:bg-white text-white dark:text-forest rounded-xl font-black text-xs uppercase tracking-widest hover:bg-accent dark:hover:bg-accent dark:hover:text-white transition-all shadow-lg group"
                   >
                   ...
                   <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 py-4 bg-white dark:bg-forest border-2 border-forest/10 dark:border-white/10 text-forest dark:text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-forest/5 dark:hover:bg-white/5 transition-all disabled:opacity-50"
                   >
                      {isExporting ? (
                         ...
                         <div className="flex items-center gap-2">
                           ...
                           <span>프리미엄 PDF 여정표 </span>
                         </div>
                      )}
                      {isExporting ? "티켓 생성 중..." : "다운로드"}
                   </button>
